...
- Si el usuario no tiene conectividad al momento de iniciar sesión por primera vez, el sistema debe informar que se requiere conexión para el registro inicial (esta es la única excepción explícita a la regla offline-first).
...
- El tutorial debe guiar al usuario por estos pasos mínimos: (1) registrar al menos una fuente de ingreso, (2) ajustar los porcentajes de distribución, (3) crear al menos una categoría de gasto con su subcategoría, (4) configurar el día de inicio de su ciclo mensual.
...
El sistema debe ofrecer tres categorías de distribución por defecto (Necesidades 50%, Deseos 30%, Ahorros 20%) y permitir ajustar los porcentajes y crear categorías adicionales.
...
Criterios de aceptación:
- Solicitar: monto, fecha, categoría de gasto, subcategoría y descripción (opcional).
...
Cuando el gasto en una categoría de distribución supera lo presupuestado, el sistema debe advertir visualmente solo en las vistas agregadas.

Criterios de aceptación:
- La advertencia visual (color/ícono) debe aparecer en el Dashboard y en la sección de Análisis.
...
El sistema emitirá notificaciones basadas en la zona horaria local del dispositivo.

Criterios de aceptación:
- Tipos de alerta: recordatorio de registro diario, presupuesto >80%, y meta al 100%.
- Las notificaciones in-app son visibles en un historial.
- Las notificaciones push (si el usuario da permiso al navegador) se enviarán a una hora predeterminada que el usuario puede configurar en sus ajustes. Se pueden prender/apagar por separado.
...
El sistema debe permitir exportar los datos del ciclo seleccionado con formato amigable.
